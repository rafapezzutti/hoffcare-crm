  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><h1 className="page-title">Profissionais de Saúde</h1><p className="page-subtitle">Médicos e Dentistas</p></div>
      ...
      <div className="card">
        <div className="search-bar">
          <div className="search-input-wrapper">
            ...
            <input className="form-control" placeholder="Buscar por nome, CPF ou CRM/CRO..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="table-container">
          <table className="table">
            <thead><tr><th>Tipo</th><th>Nome</th><th>CPF</th><th>CRM/CRO</th><th>Telefone</th><th>Email</th><th>Ações</th></tr></thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={7}><div className="empty-state"><i className="fas fa-user-md" /><p>Nenhum profissional encontrado</p></div></td></tr>}
              {filtered.map(p => (
                <tr key={p.id}>
                  <td><span className={`badge ${p.type === 'medico' ? 'badge-orange' : 'badge-blue'}`}>{p.type === 'medico' ? '🩺 Médico' : '🦷 Dentista'}</span></td>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Editar Profissional' : 'Novo Profissional'}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group"><label className="form-label">Nome Completo <span className="required">*</span></label><input className="form-control" {...f('name')} required /></div>
          <div className="form-grid form-grid-2">
            ...
            <div className="form-group"><label className="form-label">{form.type === 'medico' ? 'CRM' : 'CRO'} <span className="required">*</span></label><input className="form-control" {...f('crm_cro')} placeholder={form.type === 'medico' ? 'CRM-XX 00000' : 'CRO-XX 00000'} required /></div>
          </div>
          <div className="form-group"><label className="form-label">Data de Nascimento</label><input className="form-control" type="date" {...f('birthdate')} /></div>
          <div className="form-grid form-grid-2">
