...
// Registra automaticamente todas as requisições de escrita (POST/PUT/PATCH/DELETE)
// em audit_logs, sem precisar alterar as rotas existentes.
const pool = require('../config/db');

// Rotas que não devem gerar log
const SKIP = [
  /^\/api\/health/,
  /^\/api\/audit/,
  /^\/api\/cron/,          // jobs de sistema (sem usuário)
  /^\/api\/ai\//,          // chat de IA já tem histórico próprio
  /^\/api\/auth\/forgot-password/,
  /^\/api\/auth\/reset-password/,
];

const SENSITIVE_KEY = /password|senha|token|secret|authorization|cpf|rg/i;

// Remove senhas/tokens, trunca strings longas (fotos base64 etc.)
...
function auditLogger(req, res, next) {
  ...
  // Captura o id retornado em criações (res.json({ id: ... }))
  let responseId = null;
    ...
    try {
      ...
      pool.query(
        `INSERT INTO audit_logs
           (clinic_id, user_id, user_name, user_role, action, entity, entity_id,
