...
router.put('/:patient_id/:tooth', auth, async (req, res) => {
  ...
  const { status, procedure_name, procedure_value, notes } = req.body;
  try {
    const { rows } = await pool.query(
      `INSERT INTO odontogram_teeth
         (clinic_id, patient_id, tooth_number, status, procedure_name, procedure_value, notes, updated_at)
       ...
       DO UPDATE SET
         ...
         updated_at      = NOW()
       RETURNING *`,
      [req.user.clinic_id, patient_id, tooth, status || null, procedure_name || null,
       procedure_value ? parseFloat(procedure_value) : null, notes || null]
    );
    res.json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
