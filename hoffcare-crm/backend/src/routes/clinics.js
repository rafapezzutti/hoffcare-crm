...
router.post('/', auth, adminOnly, async (req, res) => {
  const { name, responsible_name, responsible_cpf, cep, street, number, complement, phone, email } = req.body;
  if (!name) return res.status(400).json({ error: 'Nome do consultório é obrigatório' });

  try {
    const result = await pool.query(
      `INSERT INTO clinics (name, responsible_name, responsible_cpf, cep, street, number, complement, phone, email)
  ...
  try {
    const result = await pool.query(
      `UPDATE clinics SET name=$1, responsible_name=$2, responsible_cpf=$3, cep=$4,
       street=$5, number=$6, complement=$7, phone=$8, email=$9 WHERE id=$10 RETURNING *`,
      [name, responsible_name, responsible_cpf, cep, street, number, complement, phone, email, req.params.id]
    );
    if (!result.rows[0]) return res.status(404).json({ error: 'Consultório não encontrado' });
    res.json(result.rows[0]);
  } catch (err) {
